...
(function() {
    ...
    var hostsdk = {
        ...
        // 下载书包
        downloadBookBag: function(options) {
            if (options.successCallback) window.host_sdk.successCallback = options.successCallback;
            if (options.errorCallback) window.host_sdk.errorCallback = options.errorCallback;
            if (clientTool.isApple) {
                ios_hostsdk.callHandler("downloadBookBag", {
                    bookId: options.bookId,
                    name: options.name,
                    folderId: options.folderId
                });
            } else {
                android_hostsdk.downloadBookBag(options.bookId, options.name, options.folderId);
            }
        },

        // 打开书架中的文件夹
        openShelfFolder: function(options) {
            if (options.errorCallback) window.host_sdk.errorCallback = options.errorCallback;
            if (clientTool.isApple) {
                ios_hostsdk.callHandler("openShelfFolder", {
                    name: options.name,
                    folderId: options.folderId
                });
            } else {
                android_hostsdk.openShelfFolder(options.folderId, options.name);
            }
        },
